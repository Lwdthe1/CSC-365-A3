...
public class UF<V> {
    private Object[] id;     // id[i] = parent of i
    ...
    public int find(V p) {
        ...
        return id.length;// return an out of bounds position for error checking
    }
     ...
     */
    public boolean connected(V p, V q) {
    ...
    public void union(V p, V q) {
        ...
        int j = find(q);
        if (i == j) return;
        if (i >= id.length || j >= id.length 
                || i >= rank.length || j >= rank.length) return;
